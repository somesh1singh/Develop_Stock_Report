...
Yahoo Finance is an unofficial, aggregated source and may rate-limit requests from Streamlit hosting. Deploying successfully does not establish that every ticker or statement is available. Failures show an error rather than sample data. Statements are withheld when financial currency cannot be confirmed as INR. Retrieved prices may be delayed and split-adjusted; price returns exclude cash dividends.
